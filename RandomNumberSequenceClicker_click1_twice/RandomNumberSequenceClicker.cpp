...
static const int DETECT_INTERVAL_MS = 20;
...
bool ClickSequence(const std::array<Point2D, 17>& centers, bool verbose = true) {
    if (verbose) std::cout << "Clicking 1 twice, then 2 to 16. Press F9 to emergency stop.\n";
    for (int n = 1; n <= 16; ++n) {
        ...
        // Click number 1 one extra time; all other numbers keep the same logic.
        if (n == 1) {
            if (GetAsyncKeyState(VK_F9) & 0x8000) {
                if (verbose) std::cout << "Stopped by F9.\n";
                return false;
